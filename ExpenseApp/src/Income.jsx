...
const Income = () => {
  ...
  const fetchIncomes = async (page, token, setIncomelist, setTotalPages) => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_REACT_APP_BACKEND_URL}/api/incomes?page=${page}&limit=5`,
        {
          headers: {
      ...
      setIncomelist(Array.isArray(data.response) ? data.response : []);
      setTotalPages(data.totalPages || 1);
    } catch (error) {
      if (error.response) {
        console.error("Error:", error.response.data.message || "Couldn't Find Data");
      } else {
  ...
  useEffect(() => {
    fetchIncomes(page, token, setIncomelist, setTotalPages)
              ...
              <div className="card-body">
                ...
                <div className="d-flex justify-content-end align-items-center">
                  <button
                    ...
                    onClick={() => {
                      setupdateAmount(selectIncome.amount)
